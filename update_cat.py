...
new_categoryDD = """  function categoryDD(title, id, columns, footerLink) {
    var cols = columns
      .map(function (col) {
        var links = col.links
          .map(function (link) {
    ...
    return (
      ...
      '<a href="' + footerLink[0] + '" class="nav-link flex items-center gap-1 group-hover:text-primary transition-colors" aria-expanded="false">' +
      title +
      '<span class="transform transition-transform group-hover:rotate-180">' + CHEV + '</span>' +
      '</a>' +
      '<div class="dd-panel absolute left-0 top-full mt-2 w-64 bg-card border border-border shadow-xl rounded-xl invisible opacity-0 translate-y-2 group-hover:visible group-hover:opacity-100 group-hover:translate-y-0 transition-all p-2 z-40">' +
...
text = re.sub(r'  function categoryDD\(title, id, columns, footerLink\) \{.*?(?=  function financeMenu)', new_categoryDD + '\n\n', text, flags=re.DOTALL)

with codecs.open(r'c:\Users\kolas\MyDirectory\Source Control\bitstobetter\assets\site.js', 'w', 'utf-8') as f:
    f.write(text)
print('Updated categoryDD correctly')
